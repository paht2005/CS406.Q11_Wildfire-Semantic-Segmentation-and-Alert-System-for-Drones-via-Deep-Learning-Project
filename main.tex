...
\chapter*{Work Contribution and Role Assignment}
\addcontentsline{toc}{chapter}{Work Contribution and Role Assignment}
...
\subsection{Theoretical Background and Rationale}
...
\begin{itemize}
    \item \textbf{Spatial Resampling and Intensity Normalization:} High-resolution aerial imagery inherent in UAV missions often contains redundant spatial information. Resizing images to a standardized dimension (\textbf{Resize} to $512 \times 512$) ensures computational tractability and batch consistency. Furthermore, pixel intensity normalization stabilizes gradient descent during backpropagation by ensuring a zero-centered feature distribution.
    
    \item \textbf{Stochastic Data Augmentation:} Based on the principle of maximizing dataset entropy and mitigating overfitting, we implement a comprehensive stochastic pipeline via the \textit{Albumentations} library. This simulates the stochasticity of real-world flight conditions:
    \begin{itemize}
        \item \textbf{Geometric Invariance:} Transformations such as \textit{HorizontalFlip}, \textit{VerticalFlip}, \textit{RandomRotate90}, and \textit{Rotate} ensure the model is invariant to drone heading. \textit{Perspective} and \textit{RandomScale} are critical for simulating varying camera pitch angles and flight altitudes relative to the terrain.
        \item \textbf{Spectral and Photometric Jittering:} To account for volatile solar irradiance and specular reflections, we apply \textit{RandomBrightnessContrast}, \textit{HueSaturationValue}, and \textit{RGBShift}. This compels the network to learn robust spectral signatures of fire rather than over-relying on specific illumination intensities.
        \item \textbf{Environmental and Sensor Noise Simulation:} Atmospheric haze and canopy occlusions are replicated using \textit{RandomFog} and \textit{RandomShadow}. Additionally, \textit{GaussianBlur} accounts for potential motion blur or sensor-induced thermal noise.
    \end{itemize}
    

    \item \textbf{Cross-modal Synchronization:} This involves the spatio-temporal alignment of visible-light (RGB) and long-wave infrared (LWIR) streams. Ensuring that thermal anomalies (heat signatures) and visual textures are fused at the pixel level is paramount for multi-modal feature integration.
\end{itemize}

\subsection{Functional Role and Systemic Contribution}
...
\begin{itemize}
    \item \textbf{Radiometric Refinement and Noise Mitigation:} By applying denoising filters (e.g., \textit{Gaussian Blur}) and radiometric calibration, the module suppresses sensor-induced artifacts and environmental haze, providing the U-Net architecture with high-fidelity, high-frequency features.
    \item \textbf{Structural Feature Standardization:} It homogenizes heterogeneous data streams into synchronized tensors, significantly reducing the latent space complexity for the subsequent semantic segmentation module.
    \item \textbf{Enhancing Model Generalization and Resilience:} The rigorous application of geometric and photometric transformations directly bolsters the system's operational resilience. This ensures high \textit{Intersection over Union} (IoU) and \textit{Recall} performance despite unpredictable environmental variables such as varying sunlight reflections, smoke occlusions, or complex mountainous topographies.
\end{itemize}
...
\href{https://www.sciencedirect.com/science/article/pii/S0165168421003467?via%3Dihub}{[Available]}

\bibitem{haeri2024comprehensive}
Haeri Boroujeni, M., Marzband, M., Godina, R., \& Ghosh, S. (2024). A comprehensive survey of research towards AI-enabled unmanned aerial systems in pre-, active-, and post-wildfire management. \textit{Applied Energy}, 357, 121967. \href{https://arxiv.org/abs/2401.02456}{[Available]}
...
Ronneberger, O., Fischer, P., \& Brox, T. (2015). U-Net: Convolutional networks for biomedical image segmentation. In \textit{International Conference on Medical Image Computing and Computer-Assisted Intervention (MICCAI)}, pp. 234–241. \href{https://lmb.informatik.uni-freiburg.de/people/ronneber/u-net/}{[Available]}

\bibitem{albumentations}
Buslaev, A., Iglovikov, V. I., Khvedchenya, E., Parinov, A., Kurlyandchik, M., \& Kalinin, A. A. (2020). Albumentations: Fast and flexible image augmentations. \textit{Information}, 11(2), 125. \href{https://www.scribd.com/document/804699520/1809-06839v1}{[Available]}
...
Redmon, J., \& Farhadi, A. (2018). YOLOv3: An incremental improvement. \textit{arXiv preprint arXiv:1804.02767}. \href{https://ijsrmt.com/index.php/ijsrmt/article/download/650/195/4011}{[Available]}

\bibitem{liu2022yolo}
Liu, W., Yang, X., Liu, J., \& Xu, M. (2022). Image-adaptive YOLO for object detection in adverse weather conditions. \textit{Proceedings of the AAAI Conference on Artificial Intelligence}, 36(3), 2740–2747. \href{https://www4.comp.polyu.edu.hk/~cslzhang/paper/Image_Adaptive_YOLO.pdf}{[Available]}

\bibitem{zhao2020saliency}
Zhao, Y., Xu, Z., Wang, H., Wu, H., \& Jin, Y. (2020). Saliency detection-based wildfire smoke detection using UAV imagery. In \textit{Proceedings of the IEEE International Conference on Multimedia and Expo (ICME)}, pp. 1–6. \href{http://www.ijicic.org/contents.htm}{[Available]}

\bibitem{martinez2015multi}
Martinez-de Dios, J. R., Merino, L., Caballero, F., \& Ollero, A. (2015). Multi-UAV technologies for automatic forest fire monitoring and measurement. In \textit{Proceedings of the International Conference on Computer Vision Systems (ICVS)}, pp. 207--215. \href{https://www.sciencedirect.com/science/article/abs/pii/S1051200413001462}{[Available]}
